...
export class MatchListEffects {
  loadMatchList = createEffect(() =>
    this.actions$.pipe(
      ofType(MatchListActionTypes.Load),
      switchMap(() => {
        return this.matchListService.loadAllMatchListItems().pipe(
  ...
  assignToMatchList = createEffect(() =>
    this.actions$.pipe(
      ofType(MatchListActionTypes.Assign),
      mergeMap((action: AssignToMatchList) => {
        return this.matchListService.assignToMatchList(action.payload).pipe(
          map((status) => {
  ...
  deleteMatchListItem = createEffect(() =>
    this.actions$.pipe(
      ofType(MatchListActionTypes.DeleteItem),
      switchMap((action: DeleteMatchListItem) => {
        return this.matchListService.deleteMatchListItem(action.payload).pipe(
  ...
  moveMatchListItem = createEffect(() =>
    this.actions$.pipe(
      ofType(MatchListActionTypes.MoveItem),
      switchMap((action: MoveMatchListItem) => {
        return this.matchListService.moveMatchListItem(action.payload).pipe(
